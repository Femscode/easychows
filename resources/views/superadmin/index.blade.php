								...
								<table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_sales_table">
									...
									<thead>
										...
										<tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
											...
											<th class="text-end min-w-100px">Account</th>
											<th class="text-end min-w-100px">Total Count</th>
											<th class="text-end min-w-100px">Today Count</th>
									...
									<tbody class="fw-bold text-gray-600">
										...
										<tr>
											...
											<td class="text-end" data-order="2022-01-28">
												...
												@endif
												<span class="badge badge-success">{{$user->assigned}}</span>
											</td>
											<!--end::Date Added=-->
											<!--begin::Date Modified=-->
											...
											<td class="text-end" data-order="2022-01-31">
												<form method='post' action='{{ route('update_bonus') }}'>@csrf
													<input type='hidden' value='{{ $user->id }}' name='user_id'/>
													<input type='number' value='{{ $user->bonus }}' class='form-control-sm' name='bonus'/>
												...
												<div class="card-toolbar">
													...
													<div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-800 menu-state-bg-light-primary fw-bold w-200px py-3" data-kt-menu="true" style="">
														...
														<div class="menu-item px-3">
															...
															<a onclick='return confirm("Are you sure you want to make {{$user->name}} an admin")' href="makeadmin/{{$user->id}}" class="menu-link px-3">Make Admin</a>
															@else
															<a onclick='return confirm("Are you sure you want to make {{$user->name}} a vendor")' href="/makevendor/{{$user->id}}" class="menu-link px-3">Make Vendor</a>
															@endif
														...
														<!--begin::Menu item-->
														<div class="menu-item px-3">
															<a data-bs-toggle="modal" data-id='{{$user->id}}' data-name='{{$user->name}}' data-bs-target="#kt_modal_invite_friends2" class="assign_admin menu-link flex-stack px-3">Assign Admin</a>
														</div>
														<!--end::Menu item-->
														<!--begin::Menu item-->
														<div class="menu-item px-3">
														...
														<div class="menu-item px-3 my-1">
															<a href="supercheckreview/{{$user->id}}" class="menu-link px-3">Reviews</a>
														</div>

														<div class="menu-item px-3 my-1">
															@if($user->delivery_status == 0)
															<a href="/change_delivery_status/{{ $user->id }}" class="menu-link px-3">Activate Tracking</a>
															@else
															<a href="/change_delivery_status/{{ $user->id }}" class="menu-link px-3">Deactivate Tracking</a>
															@endif
														</div>
														<div class="menu-item px-3 my-1">
															@if($user->promoted == 0)
															<a href="/promote/{{ $user->id }}" class="menu-link px-3">Promote</a>
															@else
															<a href="/promote/{{ $user->id }}" class="menu-link px-3">Depromote</a>
															@endif
														</div>
